...
def prepare_targets(frame: pd.DataFrame, config: Any) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    regression_specs = {
        "target_band_gap_ev": ("Band gap (eV)", "band_gap"),
        "target_energy_above_hull_ev_atom": ("Energy above hull (eV/atom)", "energy_above_hull"),
        "target_formation_energy_per_atom_ev": ("Formation energy per atom (eV/atom)", "formation_energy_per_atom"),
    }
    ...
    data["target_is_metal"] = data["is_metal"].astype("Int64")
    if "is_stable" in data:
        data["target_is_stable"] = data["is_stable"].astype("Int64")
    hull = pd.to_numeric(data["energy_above_hull"], errors="coerce")
